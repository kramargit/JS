...
showScore.innerText = 'Счет: ' + score;
var countBomb = 0;
...
function createBomb() {
    ...
    while (!secondBomb) { //пока 2 бомбу не создали
        // рандом
        var bomb_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var bomb_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        var twoBomb_cell = document.getElementsByClassName('cell-' + bomb_y + '-' + bomb_x)[0];
        var twoBomb_cell_classes = twoBomb_cell.getAttribute('class').split(' ');

        // проверка на змейку и корм
        if (!twoBomb_cell_classes.includes('snake-unit') && !twoBomb_cell_classes.includes('food-unit') && !twoBomb_cell_classes.includes('bomb-unit')) {
            twoBomb_cell.classList.add('bomb-unit');
            secondBomb = true;
        }
    }

    if (countBomb >= 4) {
        var removeBomb = document.getElementsByClassName('bomb-unit');
        for (var i = 0; i < removeBomb.length; i++) {
            if (i == 0 || i == 1) {
                removeBomb[i].classList.remove('bomb-unit');
            }
        }
    }
 ...
 */
function finishTheGame(bomb = false) {
    gameIsRunning = false;
    clearInterval(snake_timer);
    if (bomb) {
        alert('Вы подрвались на бомбе! Ваш результат: ' + score.toString());
    } else {
